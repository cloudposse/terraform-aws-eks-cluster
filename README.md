...
to `https://localhost/`) and/or `Error: Unauthorized`. The easiest ways to handle this is either to add `-var apply_config_map_aws_auth=false`
to the `destroy` command or to remove the ConfigMap (`...kubernetes_config_map.aws_auth[0]`) from the Terraform
state with `terraform state rm`. If still not working after removing the ConfigMap from state you can try to run `destroy -refresh=false`.
Some official opened bugs related to this are (1028)[https://github.com/hashicorp/terraform-provider-kubernetes/issues/1028] and (1479)[https://github.com/hashicorp/terraform-provider-kubernetes/issues/1479]
