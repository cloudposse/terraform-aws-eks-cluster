...
| <a name="input_addons_time_sleep_duration"></a> [addons\_time\_sleep\_duration](#input\_addons\_time\_sleep\_duration) | Time duration to delay addons creation. For example, 30s for 30 seconds or 5m for 5 minutes. Updating this value by itself will not trigger a delay. | `string` | `"3m"` | no |
